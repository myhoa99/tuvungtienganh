...
public class DatabaseUtils {
    ...
    private  final String TABLE_NEW_WORD = "tbl_new_word";
    ...
    public HashMap<String,List<NewWordModel>> getHashMapNewWord(List<NewWordModel> newWordModels) {
        HashMap<String, List<NewWordModel>> hashMap = new HashMap<>();

        for (int i = 0; i <newWordModels.size();i++) {

            hashMap.put(newWordModels.get(i).getWord(), newWordModels);
        }
        return hashMap;
    }
    ...
    public List<NewWordModel> getNewWord(){
        List<NewWordModel> newWordModelList = new ArrayList<>();
        sqLiteDatabase=myDatabase.getReadableDatabase();
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM "+ TABLE_NEW_WORD ,null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            //read data
            int id = cursor.getInt(0);
            ...
            NewWordModel newWordModel = new NewWordModel(id, word, vietnamese);
            newWordModelList.add(newWordModel);
            //next
            cursor.moveToNext();
        }
        return newWordModelList;
    }
    public NewWordModel writeNewWord(NewWordModel newWordModel, String word , String vietnamese){

        sqLiteDatabase= myDatabase.getWritableDatabase();
//        ContentValues contentValues = new ContentValues();
//        contentValues.put("word", newWordModel.getWord());
//        contentValues.put("vietnamese", newWordModel.getVietnamese());
//        sqLiteDatabase.insert(TABLE_NEW_WORD,null,contentValues);
        sqLiteDatabase.execSQL("INSERT INTO " + TABLE_NEW_WORD +"(word,vietnamese) VALUES " +"('"+word+"','"+vietnamese+"')");
